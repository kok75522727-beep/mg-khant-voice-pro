...
def _azure_endpoint(region):
    return f"https://{region}.tts.speech.microsoft.com/cognitiveservices/v1"


def generate_google_tts(text, voice_id="my-MM-NilarNeural", rate=1.0, pitch=0, api_key=None, region=None, style=""):
    ...
    with tempfile.TemporaryDirectory(prefix="mgkhant_azure_") as temp_dir:
        ...
        for index, chunk in enumerate(chunks):
            ssml = (
                ...
                'xmlns:mstts="https://www.w3.org/2001/mstts" xml:lang="my-MM">'
                f'<voice name="{escape(voice_name)}"><prosody rate="{rate:+d}%" pitch="{pitch_hz:+d}Hz">'
                f'{escape(chunk)}</prosody></voice></speak>'
            )
            response = requests.post(_azure_endpoint(area), headers=headers, data=ssml.encode("utf-8"), timeout=180)
            if response.status_code >= 400:
                ...
                raise RuntimeError(f"Azure Speech TTS error {response.status_code}: {detail}")
...
def run_tts_to_file(text, voice_id, pitch_offset=0, rate=1.0, suffix="output", api_key=None, region=None, style=""):
    audio, srt = generate_google_tts(text, voice_id=voice_id, rate=rate, pitch=pitch_offset, api_key=api_key, region=region, style=style)
