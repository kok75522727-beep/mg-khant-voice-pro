...
from voice_engine import (
    get_google_voices,
    change_tempo, get_usage_count, run_tts_to_file, MAX_TEXT_CHARS
)
...
def _speech_config():
    key = _speech_secret("AZURE_SPEECH_KEY") or _speech_secret("MICROSOFT_SPEECH_KEY")
    region = _speech_secret("AZURE_SPEECH_REGION") or _speech_secret("MICROSOFT_SPEECH_REGION")
    endpoint = _speech_secret("AZURE_SPEECH_ENDPOINT") or _speech_secret("MICROSOFT_SPEECH_ENDPOINT")
...
def tts_page():
    saved_speech_key, saved_speech_region, saved_speech_endpoint = _speech_config()
    if not saved_speech_key or not saved_speech_region:
        st.error("⚠️ Azure Speech Secret မတွေ့ပါ။ Streamlit Secrets ထဲမှာ AZURE_SPEECH_KEY နှင့် AZURE_SPEECH_REGION ထည့်ပါ။")
        return
    ...
    with st.container(border=True):
        ...
        st.markdown(
            f"<div style='text-align:right; color:#64748b; font-size:13px; margin-top:-8px; margin-bottom:12px;'>လက်ရှိစာလုံးရေ — <b>{len(text):,}</b> / {MAX_TEXT_CHARS:,} လုံး</div>",
            unsafe_allow_html=True,
        )
        
        render_section("2", "အသံအမျိုးအစား ရွေးချယ်ခြင်း")
        try:
            google_voices = get_google_voices(limit=12)
        except Exception as exc:
            st.error(f"❌ အသံစာရင်း မရပါ: {exc}")
            return
        ...
        with st.spinner("⏳ အသံဖိုင် ဖန်တီးနေပါသည်... စာလုံးရေများလို့ ခဏကြာနိုင်ပါတယ်။"):
                try:
                    ...
                    audio_path, srt_path = run_tts_to_file(
                        ...
                        rate=speed,
                        suffix="custom",
                        api_key=saved_speech_key,
                        region=saved_speech_region,
                        endpoint=saved_speech_endpoint,
                    )
                    ...
                    if "AZURE_QUOTA_EXCEEDED" in error_text or "429" in error_text or "quota" in error_text.lower():
                        st.error("❌ Azure Speech quota ပြည့်သွားပါပြီ။")
                    elif "Key" in error_text or "Region" in error_text:
                        st.error(f"❌ Azure Secret setting ကို စစ်ပါ: {error_text}")
                    else:
